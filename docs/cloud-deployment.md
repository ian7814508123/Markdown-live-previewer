...
本文件提供多種**免費或低成本**的雲端部署選項,確保你的 Markdown Live Previewer 可以公開存取,且前端畫面與本地完全一致。

---

## 🎯 為什麼 GitHub Pages 會有問題?

### 問題分析
GitHub Pages 部署時,你的應用需要使用子路徑 `/Markdown-live-previewer/`,但這會導致:

1. **資源路徑錯誤**:所有 CSS/JS 檔案需要加上前綴
2. **靜態路由問題**:SPA 路由可能無法正確處理子路徑
3. **不一致性**:`npm run preview` 使用根路徑但 GitHub Pages 使用子路徑
