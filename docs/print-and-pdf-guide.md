...
## 4. 手動分頁指令 (Manual Page Break)
除了自動分頁偵測外，使用者可以在 Markdown 原始碼中加入特定指令來強制換頁，這在需要控制章節起始位置時非常有用。
...
### 使用方式：
1. 在想要分頁的地方新起一行。
2. 輸入指令（例如：`\pagebreak`），確保該行沒有其他文字。
3. 在編輯器的實時預覽中，您會看到一條帶有 **"FORCE PAGE BREAK"** 字樣的虛線，這表示分頁指令已生效。
4. 在執行「列印 / PDF」或匯出時，預覽中的虛線位置將觸發物理分頁。
